      ...
      <el-table-column label="审核状态" align="center">
        <template slot-scope="scope">
          <el-tag effect="dark" style="border: 0;" :type="scope.row.check_status === 0 ? '' : scope.row.check_status === 1 ? 'success' : 'danger'">
            {{ scope.row.check_status === 0 ? '未审核' : scope.row.check_status === 1 ? '通过审核' : '拒绝审核' }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="操作" width="300">
        <template slot-scope="scope">
          <div class="operation-buttons">
            <el-button type="success" :disabled="scope.row.check_status !== 0" @click="check(scope.row, scope.$index)">
              通过审核
            </el-button>
            <el-button type="danger" :disabled="scope.row.check_status !== 0" @click="noCheck(scope.row, scope.$index)">
              拒绝审核
            </el-button>
        ...
        <el-row style="margin-bottom: 20px">
          <el-col :span="12">
            <el-form-item label="拒绝理由" :label-width="formLabelWidth">
              <el-input v-model="cash.desc" autocomplete="off" />
            </el-form-item>
          </el-col>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    check(item) {
      ...
      this.cash = item
      this.cash.check_status = 1
    },
    postCheck() {
      postAdd(this.cash).then(res => {
        if (res.code === 200) {
